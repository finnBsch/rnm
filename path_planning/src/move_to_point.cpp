...
void callback(const std_msgs::Float64MultiArray& msg) {
  ...
  output.trajectory.calculate(input, 0.001);
  vector<array<double, 7>> goal_trajectory;
  trajectory_msgs::JointTrajectory traj_msg;

  size_t sample_count = std::ceil(output.getDuration() / 0.001);
  array<double, 7> one_point;
  for (size_t sample = 0; sample <= sample_count; ++sample)
  {
    // always sample the end of the trajectory as well
    double t = std::min(output.getDuration(), sample * 0.001);
    Vector<double,7> np;
    output.trajectory.at_time(t,np);
