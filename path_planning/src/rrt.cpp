...
vector<joint_angles> rrt::return_planned_nodes() {
    rrt_node* current_node = goal_node;
    vector<joint_angles> ret;
    while(current_node!= nullptr) {
        ret.push_back(current_node->get_angles());
...
// eval goal-path
trajectoryStruct rrt::return_goal_path() {
    rrt_node* current_node = goal_node;
    trajectoryStruct goal_path;
    waypointStruct wp;
    ...
    while(current_node!= nullptr) {
        ...
        bool change = (counter == 0);
        for(int i = 0; i<joints.size(); i++){

            joints[i].at(counter) = temp[i];
            if(counter>0){
                if(abs(joints[i].at(counter-1) - joints[i].at(counter)) > 0.005){
                    change = true;
                }
            }
        }
        if(change == false){
            ROS_ERROR("TOO SMALL STEPS");
        }
        counter++;
        current_node = current_node->get_parent();
    }
    ...
    if(trajectory.isValid()) {
        ...
        for (size_t sample = 0; sample <= sample_count; ++sample)
        {
            ...
            auto vel = trajectory.getVelocity(t);
            auto acc = trajectory.getAcceleration(t);
            for (size_t j = 0; j < 6; ++j)
            {
                wp.position[j] = pos[j];
                wp.velocity[j] = vel[j];
                wp.acceleration[j] = acc[j];
            }
    ...
    Point start  = get_end_effector(traj.waypoints[0].position);
    ...
    for(int i = 0; i<traj.waypoints.size(); i++){
        Point temp_ = get_end_effector(traj.waypoints[i].position);
        for (size_t j = 0; j < 3; ++j){
            traj.waypoints[i].pos3d[j] = temp_[j];
        }
    }
    return traj;
}
